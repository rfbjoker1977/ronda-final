...
const NEWS_CATEGORIES=["Noticias","Clausulazos","Enfermería","Novedades","Datos"];
...
const DEFAULT_DATA={league:"RONDA FINAL",season:"2026 / 2027",welcome:"",clubs:DEFAULT_CLUBS,cups:{apertura:emptyCup(),clausura:emptyCup()},leaguesCompleted:{apertura:false,clausura:false,general:false},news:[]};
...
function loadData(){
  try{
    ...
    clubs=DEFAULT_CLUBS.map((fallback,i)=>{const club={...fallback,...(clubs?.[i]||{})};if(club.pointsGeneral===0&&club.points)club.pointsGeneral=club.points;club.badges=Array.from({length:4},(_,b)=>({...fallback.badges[b],...(club.badges?.[b]||{})}));club.titleWins=Array.from({length:5},(_,t)=>Math.max(0,Math.min(10,Number(club.titleWins?.[t])||0)));club.players=Array.from({length:25},(_,p)=>({...fallback.players[p],...(club.players?.[p]||{})}));club.matchdays=Array.from({length:38},(_,j)=>Number(club.matchdays?.[j])||0);club.playerHistory=Array.isArray(club.playerHistory)?club.playerHistory:[];club.lineups=Array.from({length:38},(_,j)=>Array.from({length:11},(_,p)=>({name:club.lineups?.[j]?.[p]?.name||"",points:Number(club.lineups?.[j]?.[p]?.points)||0})));club.formations=Array.from({length:38},(_,j)=>club.formations?.[j]||"1-4-3-3");club.transfers=Array.isArray(club.transfers)?club.transfers:[];club.initialBalance=Number(club.initialBalance)||0;return club});
    const merged={...DEFAULT_DATA,...saved,clubs};merged.cups={apertura:mergeCup(saved.cups?.apertura),clausura:mergeCup(saved.cups?.clausura)};merged.leaguesCompleted={...DEFAULT_DATA.leaguesCompleted,...(saved.leaguesCompleted||{})};merged.news=Array.isArray(saved.news)?saved.news.map(item=>({...item,category:NEWS_CATEGORIES.includes(item.category)?item.category:"Noticias"})):[];if(merged.season==="2026 / 27")merged.season="2026 / 2027";if(merged.league==="UNIVERSO")merged.league="RONDA FINAL";return merged;
...
function safeNewsUrl(value){const url=String(value||"").trim();return /^(https?:\/\/|#)/i.test(url)?url:""}
function renderNews(){const grid=document.getElementById("homeNewsGrid");if(!data.news.length){grid.innerHTML=`<div class="news-empty"><span>RF</span><div><strong>PRÓXIMAMENTE</strong><p>Los comunicados oficiales aparecerán aquí.</p></div></div>`;return}grid.innerHTML=data.news.map((item,index)=>{const url=safeNewsUrl(item.link),tag=esc(item.category||"NOTICIA");return `<article class="news-card news-card-${index%3} ${item.image?'has-news-image':''}">${item.image?`<img class="news-card-image" src="${esc(item.image)}" alt="${esc(item.title||'Publicación de Ronda Final')}">`:""}<div class="news-card-content"><div class="news-meta"><span>${tag}</span><time>${esc(item.date||"FECHA POR CONFIRMAR")}</time></div><h3>${esc(item.title||"Sin título")}</h3><p>${esc(item.body||"")}</p>${url?`<a href="${esc(url)}" ${url.startsWith('http')?'target="_blank" rel="noopener"':''}>MÁS INFORMACIÓN →</a>`:""}</div></article>`}).join("")}
function newsEditorRow(item={category:"Noticias",date:"",title:"",body:"",link:"",image:""}){return `<div class="news-editor-row"><div class="news-editor-top"><label>Categoría<select data-news-field="category">${NEWS_CATEGORIES.map(value=>`<option ${item.category===value?'selected':''}>${value}</option>`).join("")}</select></label><label>Fecha<input data-news-field="date" type="date" value="${esc(item.date)}"></label><button class="news-remove" type="button" aria-label="Eliminar publicación">×</button></div><div class="news-image-editor">${item.image?`<img class="news-image-preview" src="${esc(item.image)}" alt="Vista previa">`:""}<label>Imagen de la publicación · 256 × 256<input data-news-image type="file" accept="image/png,image/jpeg,image/webp"></label><input data-news-field="image" type="hidden" value="${esc(item.image||'')}">${item.image?`<button class="news-image-remove" type="button">QUITAR IMAGEN</button>`:""}</div><label>Título<input data-news-field="title" value="${esc(item.title)}" maxlength="80"></label><label>Texto<textarea data-news-field="body" rows="3" maxlength="320">${esc(item.body)}</textarea></label><label>Enlace opcional<input data-news-field="link" value="${esc(item.link)}" placeholder="https://..."></label></div>`}
function renderNewsAdmin(){document.getElementById("newsAdminList").innerHTML=data.news.map(newsEditorRow).join("")}
function render(){
...
document.getElementById("editorForm").addEventListener("submit",async e=>{
  ...
  data.news=[...document.querySelectorAll(".news-editor-row")].map(row=>Object.fromEntries([...row.querySelectorAll("[data-news-field]")].map(input=>[input.dataset.newsField,input.value.trim()]))).filter(item=>item.title||item.body);readCupEditors();
