...
const DEFAULT_CLUBS=Array.from({length:12},(_,i)=>({
  ...
  players:Array.from({length:18},()=>({name:"",position:"Medio",marketValue:"",clause:"",points:0})),
...
const DEFAULT_DATA={league:"RONDA FINAL",season:"2026 / 2027",welcome:"",clubs:DEFAULT_CLUBS,cups:{apertura:emptyCup(),clausura:emptyCup()},leaguesCompleted:{apertura:false,clausura:false,general:false}};
...
function loadData(){
  try{
    ...
    clubs=DEFAULT_CLUBS.map((fallback,i)=>{const club={...fallback,...(clubs?.[i]||{})};if(club.pointsGeneral===0&&club.points)club.pointsGeneral=club.points;club.badges=Array.from({length:4},(_,b)=>({...fallback.badges[b],...(club.badges?.[b]||{})}));club.titleWins=Array.from({length:5},(_,t)=>Math.max(0,Math.min(10,Number(club.titleWins?.[t])||0)));club.players=Array.from({length:18},(_,p)=>({...fallback.players[p],...(club.players?.[p]||{})}));club.matchdays=Array.from({length:38},(_,j)=>Number(club.matchdays?.[j])||0);club.playerHistory=Array.isArray(club.playerHistory)?club.playerHistory.map(player=>({name:player.name||"",entryDate:player.entryDate||"",exitDate:player.exitDate||"",legacyPoints:Number(player.legacyPoints??player.pointsPerMatchday??player.points)||0,matchdayPoints:Array.from({length:38},(_,j)=>player.matchdayPoints?.[j]??null)})):[];club.lineups=Array.from({length:38},(_,j)=>Array.from({length:11},(_,p)=>({name:club.lineups?.[j]?.[p]?.name||"",points:Number(club.lineups?.[j]?.[p]?.points)||0})));club.formations=Array.from({length:38},(_,j)=>club.formations?.[j]||"1-4-3-3");club.transfers=Array.isArray(club.transfers)?club.transfers:[];club.initialBalance=Number(club.initialBalance)||0;return club});
    const merged={...DEFAULT_DATA,...saved,clubs};delete merged.news;merged.cups={apertura:mergeCup(saved.cups?.apertura),clausura:mergeCup(saved.cups?.clausura)};merged.leaguesCompleted={...DEFAULT_DATA.leaguesCompleted,...(saved.leaguesCompleted||{})};if(merged.season==="2026 / 27")merged.season="2026 / 2027";if(merged.league==="UNIVERSO")merged.league="RONDA FINAL";return merged;
...
function cupTieMarkup(label,tie){return `<article class="cup-tie"><div class="cup-tie-head"><span>${label}</span><small>${esc(tie.date||"FECHAS PENDIENTES")}</small></div>${[0,1].map(team=>`<div class="cup-team">${tie.teams[team]!==null?crestMarkup(data.clubs[tie.teams[team]],tie.teams[team],"cup-team-crest"):`<i>?</i>`}<strong>${esc(clubName(tie.teams[team]))}</strong><b>IDA <em>${scoreText(tie.scores[0][team])}</em><br>VUELTA <em>${scoreText(tie.scores[1][team])}</em></b></div>`).join("")}<div class="cup-winner">CLASIFICADO · <strong>${esc(clubName(tie.winner,"POR DECIDIR"))}</strong></div></article>`}
function cupStageMarkup(number,spec,cup){return `<section class="cup-stage ${spec.key}"><div class="cup-stage-title"><span>${number}</span><div><small>DOBLE PARTIDO</small><h3>${spec.title}</h3></div></div><div class="cup-ties">${cup.stages[spec.key].map((tie,i)=>cupTieMarkup(`${spec.prefix} ${i+1}`,tie)).join("")}</div></section>`}
function renderCupFormats(){document.querySelectorAll(".cup-format").forEach(container=>{const key=container.dataset.cup.includes("Apertura")?"apertura":"clausura",cup=data.cups[key],champion=cup.stages.final[0].winner;container.innerHTML=`<section class="cup-rules"><div><small>FORMATO OFICIAL</small><h3>DE 12 A 1 CAMPEÓN</h3></div><ol><li><b>12 equipos</b> disputan la ronda previa.</li><li><b>6 ganadores</b> pasan directamente.</li><li><b>2 mejores perdedores</b> completan los cuartos.</li><li><b>8 equipos</b> disputan el cuadro final.</li></ol><span class="dates-pending">CALENDARIO · EDITABLE</span></section>${cupStageMarkup("01",CUP_STAGE_SPECS[0],cup)}<section class="qualification-strip qualification-new"><div><small>CLASIFICAN A CUARTOS</small><strong>6 GANADORES</strong></div><i>+</i><div><small>REPESCA</small><strong>${cup.bestLosers.map(i=>esc(clubName(i,"POR DECIDIR"))).join(" · ")}</strong></div><i>=</i><div class="qualification-total"><small>CUADRO FINAL</small><strong>8 EQUIPOS</strong></div></section>${CUP_STAGE_SPECS.slice(1).map((spec,i)=>cupStageMarkup(String(i+2).padStart(2,"0"),spec,cup)).join("")}<section class="cup-champion"><span>★</span><small>CAMPEÓN DE ${esc(container.dataset.cup.toUpperCase())}</small><strong>${esc(clubName(champion,"POR DECIDIR"))}</strong><p>La copa espera dueño.</p></section>`})}
...
function playersEditorMarkup(club){return `<details class="squad-editor" open><summary>Plantilla de 18 jugadores</summary><p class="players-help">Crea aquí la plantilla del club. Estos son los únicos futbolistas disponibles para las alineaciones y los no alineados sumarán 0 puntos.</p><div class="players-editor">${club.players.map((player,p)=>`<div class="player-editor-row" data-player="${p}"><span>${String(p+1).padStart(2,"0")}</span><label>Jugador<input data-player-field="name" value="${esc(player.name)}" maxlength="40" placeholder="Nombre del jugador"></label><label>Posición<select data-player-field="position">${["Portero","Defensa","Medio","Delantero"].map(pos=>`<option ${player.position===pos?'selected':''}>${pos}</option>`).join("")}</select></label><label>Valor mercado<input data-player-field="marketValue" value="${esc(player.marketValue)}" maxlength="20"></label><label>Cláusula<input data-player-field="clause" value="${esc(player.clause)}" maxlength="20"></label><label>Puntos acumulados<input data-player-field="points" type="number" value="${Number(player.points)||0}" readonly></label></div>`).join("")}</div></details>`}
...
function matchdaysEditorMarkup(club){const block=(start,end,title)=>`<div class="admin-round"><span class="admin-round-title">${title}</span>${club.matchdays.slice(start,end).map((points,offset)=>{const j=start+offset;return `<label>Jornada ${j+1}<input data-matchday="${j}" type="number" step="0.01" value="${Number(points)||0}" readonly></label>`}).join("")}</div>`;return `<details class="matchdays-editor"><summary>Totales por jornada · automático</summary><p class="matchday-help">Cada total es la suma de los 11 jugadores alineados. No necesita edición manual.</p><div class="matchday-inputs">${block(0,19,"Apertura · J1–J19")}${block(19,38,"Clausura · J20–J38")}</div></details>`}
...
function validateLeagueData(){
  for(const club of data.clubs){
    for(let jornada=0;jornada<club.lineups.length;jornada++){
      const names=club.lineups[jornada].map(player=>playerKey(player.name)).filter(Boolean);
      if(new Set(names).size!==names.length)throw new Error(`${club.name}: hay jugadores repetidos en la jornada ${jornada+1}.`);
    }
    for(const player of club.playerHistory)if(player.entryDate&&player.exitDate&&player.exitDate<player.entryDate)throw new Error(`${club.name}: la salida de ${player.name} es anterior a su entrada.`);
  }
}
...
function render(){
  data.clubs.forEach(club=>syncClubHistory(club));
...
function renderTeamEditors(){
  document.getElementById("teamInputs").innerHTML=data.clubs.map((club,i)=>`<div class="team-editor" data-editor="${i}"><span class="team-editor-title">CLUB ${String(i+1).padStart(2,"0")}</span>${club.crest?`<img class="upload-preview" src="${esc(club.crest)}" alt="">`:""}<label>Nombre<input data-field="name" value="${esc(club.name)}" maxlength="32"></label><label>Manager<input data-field="manager" value="${esc(club.manager)}" maxlength="40"></label><div class="points-editor"><label>Liga Apertura<input data-field="pointsApertura" type="number" min="0" step="1" value="${Number(club.pointsApertura)||0}"></label><label>Liga Clausura<input data-field="pointsClausura" type="number" min="0" step="1" value="${Number(club.pointsClausura)||0}"></label><label>Liga Ronda Final<input data-field="pointsGeneral" type="number" min="0" step="1" value="${Number(club.pointsGeneral)||0}"></label></div>${playersEditorMarkup(club)}${lineupEditorMarkup(club,i)}${transfersEditorMarkup(club)}${matchdaysEditorMarkup(club)}${historyEditorMarkup(club)}<details class="title-wins-editor"><summary>Títulos e insignias oficiales</summary>${COMPETITION_NAMES.map((name,t)=>`<label>${name}<input data-title-win="${t}" type="number" min="0" max="10" value="${club.titleWins[t]}"></label>`).join("")}</details><label>Escudo 512 × 512<input data-crest-input="${i}" type="file" accept="image/png,image/jpeg,image/webp"></label><p class="upload-help">PNG, JPG o WebP. Recomendado: 512 × 512 y menos de 1 MB.</p><details class="badge-editor"><summary>Otras insignias del club</summary>${club.badges.map((badge,b)=>`<div class="badge-field">${badge.image?`<img src="${esc(badge.image)}" alt="">`:""}<label>Insignia ${b+1}<input data-badge-name="${b}" value="${esc(badge.name)}" placeholder="Ej: Equipo Fundador" maxlength="32"></label><label>Icono<input data-badge-input="${i}:${b}" type="file" accept="image/png,image/jpeg,image/webp"></label></div>`).join("")}</details></div>`).join("");
...
document.getElementById("teamInputs").addEventListener("change",e=>{const select=e.target.closest("[data-lineup-select]");if(select){const editor=select.closest("[data-editor]"),club=data.clubs[Number(editor.dataset.editor)],jornada=Number(select.value);editor.querySelector(".admin-lineup-grid").innerHTML=lineupAdminRows(club,jornada);editor.querySelector("[data-formation-select]").value=club.formations[jornada];return}const editor=e.target.closest("[data-editor]");if(!editor)return;const club=data.clubs[Number(editor.dataset.editor)],jornada=Number(editor.querySelector("[data-lineup-select]")?.value||0);const formation=e.target.closest("[data-formation-select]");if(formation){club.formations[jornada]=formation.value;return}const field=e.target.closest("[data-lineup-field]");if(field){const row=field.closest("[data-lineup-player]"),player=club.lineups[jornada][Number(row.dataset.lineupPlayer)];player[field.dataset.lineupField]=field.dataset.lineupField==="points"?Number(field.value)||0:field.value}});
document.getElementById("teamInputs").addEventListener("input",e=>{const editor=e.target.closest("[data-editor]");if(!editor)return;const club=data.clubs[Number(editor.dataset.editor)],rosterField=e.target.closest("[data-player-field]");if(rosterField){const row=rosterField.closest("[data-player]"),player=club.players[Number(row.dataset.player)];player[rosterField.dataset.playerField]=rosterField.dataset.playerField==="points"?Number(rosterField.value)||0:rosterField.value;return}const input=e.target.closest("[data-lineup-field]");if(!input)return;const playerRow=input.closest("[data-lineup-player]"),jornada=Number(editor.querySelector("[data-lineup-select]").value),player=club.lineups[jornada][Number(playerRow.dataset.lineupPlayer)];player[input.dataset.lineupField]=input.dataset.lineupField==="points"?Number(input.value)||0:input.value});
document.getElementById("teamInputs").addEventListener("click",e=>{const preset=e.target.closest("[data-badge-preset]");if(preset){const club=data.clubs[Number(preset.dataset.club)],badge=club.badges.find(b=>!b.name&&!b.image),selected=BADGE_PRESETS[preset.dataset.badgePreset];if(!badge){alert("Este club ya tiene ocupadas sus cuatro insignias.");return}badge.name=selected.name;badge.image=selected.image;renderTeamEditors();renderBadgeLibraries();return}const transferAdd=e.target.closest(".transfer-add");if(transferAdd){transferAdd.previousElementSibling.insertAdjacentHTML("beforeend",transferEditorRow());return}const transferRemove=e.target.closest(".transfer-remove");if(transferRemove)transferRemove.closest(".transfer-editor-row").remove()});
